...
export default function StoryGenerator({ initialDestinationId }: StoryGeneratorProps) {
  ...
  const renderCanvas = () => {
    ...
    ctx.fillStyle = `hsl(${getComputedStyle(document.documentElement).getPropertyValue('--homa-blue')})`;
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value="content">
                {storyContent ? (
                  <div className="space-y-6">
                    {styleRecommendation && (
                      <div className="p-3 rounded-lg text-white text-sm bg-homa-blue">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-4">
                        {storyContent.hashtags.map((tag, index) => (
                          <div key={index} className="px-2 py-1 rounded-full text-sm text-white bg-homa-blue">
                            #{tag}
                          </div>
                        ))}
                      </div>
            ...
            {selectedDestination && (
              <div className="space-y-6">
                    ...
                    {['original', 'vintage', 'monochrome', 'vivid', 'muted'].map((filterOption) => (
                      <button
                        ...
                        className={`p-2 text-xs rounded-md ${
                          filter === filterOption 
                            ? 'bg-homa-blue text-white font-medium' 
                ...
                <div className="flex gap-4">
                  ...
                  <Button 
                    className="flex-1 bg-homa-blue hover:opacity-90 text-white"
                    disabled={!storyContent}
                  >
